...
Item {
    ...
    Loader {
        ...
        onLoaded: {
            sourceComponent.showMargins = Qt.binding(function() {
                return root.showMargins
            });
            sourceComponent.imagePath = root.imagePath;
            if ("frameSvgPrefixes" in sourceComponent) {
                sourceComponent.frameSvgPrefixes = root.frameSvgPrefixes
            }
        }
    }
    Rectangle {
